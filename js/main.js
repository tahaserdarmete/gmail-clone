...
uiElement.mailArea.addEventListener("click", updateMail);
...
uiElement.navigation.addEventListener("click", (e) => {
  ...
  if (categoryName === "Yıldızlananlar") {
    ...
    renderMails(uiElement.mailArea, staredMails);
  } else {
    renderMails(uiElement.mailArea, mails);
  }
});
...
uiElement.searchForm.addEventListener("submit", (e) => {
  ...
  const filteredMails = mails.filter((i) =>
    i.reciver.toLowerCase().includes(query.toLowerCase())
  );

  renderMails(uiElement.mailArea, filteredMails);
});
